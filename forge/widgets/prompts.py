...
class PromptWidget(QWidget):
    ...
    def draw_ui(self):
        ...
        if not self.settings_controller.get('hide_ui.styles'):
            self.style_collapsible = self.styles_control()
            self.layout().addWidget(self.style_collapsible)
    ...
    def load_prompt(self):
        ...
        try:
            ...
            if self.settings_controller.has('prompts.share_prompts') and self.settings_controller.get('prompts.share_prompts') and self.mode not in excluded:
    ...
    def set_generation_data(self, data: dict) -> None:
        if "prompt" in data:
            self.prompt_text_edit.setPlainText(data["prompt"])
        if "negative_prompt" in data:
            self.negative_prompt_text_edit.setPlainText(data["negative_prompt"])
        if "styles" in data:
            styles = data["styles"]
            for index in range(self.style_name_list.count()):

...
class SeedWidget(QWidget):
    ...
    def draw_ui(self):
        ...
        if self.variables['seed'] != -1:
            self.seed_edit.setText(str(self.variables['seed']))
        ...
        if self.variables['subseed'] != -1:
            self.subseed_edit.setText(str(self.variables['subseed']))
        ...
        self.percentage.setText('%s%%' % int(self.variables['subseed_strength'] * 100))
    ...
    def set_generation_data(self, data: dict) -> None:
        if "seed" in data:
            seed = data["seed"]
            self.seed_edit.setText("" if seed == -1 else str(seed))
            self._update_variable("seed", self.seed_edit.text())
        if "subseed" in data:
            subseed = data["subseed"]
            self.subseed_edit.setText("" if subseed == -1 else str(subseed))
            self._update_variable("subseed", self.subseed_edit.text())
        if "subseed_strength" in data:
            strength = data["subseed_strength"]
            self.subseed_strength_slider.setValue(int(strength * 100))
            self._update_variable("subseed_strength", self.subseed_strength_slider.value())
